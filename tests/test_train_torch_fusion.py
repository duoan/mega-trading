...
from marketfm.train.torch_fusion import (
    TorchFusionDataset,
    TorchFusionModel,
...
class TorchFusionTests(unittest.TestCase):
    def test_dataset_pads_streams_and_encodes_labels(self) -> None:
        ...
        dataset = TorchFusionDataset(rows, price_window_size=4, fundamental_size=3, evidence_size=5)
        item = dataset[0]
    ...
    def test_model_forward_returns_two_prediction_heads(self) -> None:
        model = TorchFusionModel(price_window_size=4, fundamental_size=3, evidence_size=5, hidden_dim=8)
        batch = {
    ...
    def test_trainer_consumes_stream_shards_and_writes_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = TorchFusionTrainer(
                store,
                TorchFusionTrainConfig(run_id="torch-fusion-test", max_steps=2, hidden_dim=8, batch_size=2),
            ).train("stage=05_shards/mixture=public/samples.jsonl")
            metrics = store.read_jsonl("runs/torch-fusion-test/metrics.jsonl")
            manifest = store.read_manifest(result.manifest_path)

            self.assertEqual(result.steps, 2)
            self.assertEqual(metrics[-1]["stage"], "torch_fusion")
            self.assertIn("loss", metrics[-1])
            self.assertTrue((Path(tmp) / result.checkpoint_path).exists())
            self.assertEqual(manifest.metadata["stage"], "torch_fusion")
