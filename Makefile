...
demo:
	...
	uv run mega-trading build data.data_dir=.mega-trading/demo data.source=fixture build.block_size=4 build.stride=2 build.min_events_per_ticker=2
...
prep-binance-modal:
	uv run mega-trading ingest --config configs/ingest-binance-modal-prep.toml
	uv run mega-trading build --config-name binance-modal-prep
